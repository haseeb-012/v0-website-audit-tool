...
export function Hero() {
  ...
  return (
    <section className="relative flex min-h-[calc(100vh-4rem)] items-center overflow-hidden bg-slate-deep px-5 py-14 md:px-12 md:py-20">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-16 lg:grid-cols-2 lg:gap-20">
        <div>
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-accent-bright/25 bg-accent-bright/10 px-4 py-1.5 text-[12px] font-bold uppercase tracking-[1.2px] text-[#4ade80]">
            <span className="qa-blink size-[7px] rounded-full bg-[#4ade80]" />
            9 Years of QA Expertise — Now Automated
          </div>

          <h1 className="font-heading text-[clamp(2.375rem,4.8vw,3.875rem)] font-black leading-[1.05] tracking-[-0.02em] text-balance text-white">
            Is your website{" "}
            <span className="text-accent-bright">actually working</span> for
            your customers?
          </h1>

          <p className="mt-5 max-w-lg text-[17px] leading-[1.75] text-white/60">
          ...
          <div className="mt-10 rounded-2xl border border-white/15 bg-white/5 p-5">
            <div className="flex flex-col gap-2.5 sm:flex-row">
              <input
                ...
                className={cn(
                  "flex-1 rounded-xl border border-white/15 bg-white/10 px-5 py-4",
                  "font-mono text-sm text-white outline-none transition-all placeholder:text-white/30",
              ...
              <button
                ...
                onClick={submit}
                className={cn(
                  "inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-xl bg-accent-bright px-7 py-4",
                  "text-[15px] font-extrabold tracking-wide text-white transition-all",
                  "hover:-translate-y-0.5 hover:bg-accent-emerald hover:shadow-lg hover:shadow-accent-bright/35",
                )}
              >
            ...
            <div className="mt-3.5 flex flex-wrap items-center gap-4">
              ...
              <TrustItem label="Free preview in 60 seconds" />
...
function TrustItem({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-1.5 text-xs font-medium text-white/45">
      <span className="inline-flex size-4 items-center justify-center rounded-full bg-accent-bright/20 text-[9px] font-extrabold text-[#4ade80]">
...
function StatItem({ value, label }: { value: string; label: string }) {
  return (
    <div className="border-l-2 border-white/10 pl-4">
      <span className="block font-heading text-[26px] font-black leading-none text-white">
        {value}
      </span>
